...
def read_config(file):
    ...
    for line in open(file, "r"):
        ...
        if line[0] == "[" and line[-1] == "]":
            ...
            continue
        if block == "":
            raise "Configuration error: setting outside block [%s] %s" % (
                file,
                line,
            )
        items = line.split("=", 1)
        if len(items) != 2:
            raise "Configuration error: setting without equals sign [%s] %s" % (
                file,
                line,
            )
    ...
    for key, value in blocks["options"].items():
        ...
        if key == "syslog":
            ...
            raise "Configuration error: Invalid option [%s] %s=%s" % (
                file,
                key,
                value,
            )
